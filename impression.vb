...
Public Class impression
    ...
    Private Sub GetBilledItems()
        ...
        If (Integer.Parse(txtId.Text) >= 10000000) Then
            Try
                ...
                myConn.Close()
                myConn.Open()
                qry = "SELECT encounter_nr, encounter_date, boncfg, encounter_type, contact_name  FROM caredb.care_encounter WHERE pid=" & CULng(txtId.Text) & _
                ...
                myConn.Open()
                qry = "SELECT bill_no FROM `care_billing_bill` WHERE encounter_nr=" & Encounter ' & " ORDER BY bill_no,date DESC"
                Comd = New MySqlCommand(qry, myConn)
                'Get the encounter number
                PrescReader = Comd.ExecuteReader

                'recuperation du nombre de facture pour la rencontre
                Dim num As Integer = PrescReader.FieldCount

                If num = 0 Then
                    MsgBox("Pas de Facture pour ce patient")
                    myConn.Close()
                    Exit Sub
                
                Else
                    Dim ltfact As New DataTable
                    myConn2.Open()
                    qry = "SELECT bill_no,date,agent FROM `care_billing_bill` WHERE encounter_nr=" & Encounter & " ORDER BY bill_no DESC, date DESC" ' LIMIT 1"
                    Comd2 = New MySqlCommand(qry, myConn2)
                    dtaAdapter.SelectCommand = Comd2
                    dtaAdapter.Fill(ltfact)
                    myConn2.Close()

                    'affichage de la bd

                    listfacture.Visible = True
                    listfacture.DataSource = ltfact
    ...
    Private Sub btnValidate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnValidate.Click
        ...
        Else
            ...
            If ispaid() Then
                ...
                miseajour_care_billing_bill_item_paid()
    ...
    Private Sub affiche_element_partiel()
        ...
        Try
            myConn.Open()
            dtaDataTable.Reset()
            qry = "SELECT `article` as Article,`unit_cost` as Prix,`units` as Qte,`amount` as Total  FROM `care_billing_bill_item` WHERE `encounter_nr`=" & Encounter & "  AND  `islab`=0 AND status='paid' OR status='partial' AND `bill_no`=" & testbill & " ORDER BY  article" 'AND `bill_number`=0"
            Comd = New MySqlCommand(qry, myConn)
            ...
            If ilyalabetproduit() Then
                myConn.Open()
                qry = "SELECT SUM(`amount`) as Total  FROM `care_billing_bill_item` WHERE `encounter_nr`=" & Encounter & "  AND  `islab`=0 AND status='paid' OR status='partial' AND `bill_no`=" & testbill  'AND `bill_number`=0"
                Comd = New MySqlCommand(qry, myConn)
                PrescReader = Comd.ExecuteReader
                While PrescReader.Read()
                    newListRow = dtaDataTable.NewRow
    ...
    Private Sub listefacture_doubleclick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles listfacture.CellDoubleClick

        testbill = listfacture.Rows(listfacture.CurrentRow.Index).Cells(0).Value
        agentdefacturation = listfacture.Rows(listfacture.CurrentRow.Index).Cells(2).Value.ToString
        billnum = testbill
